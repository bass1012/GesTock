  ...
  return (
    ...
    >
      ...
      <div className="flex h-16 items-center justify-between px-4 border-b border-white/10">
        {!collapsed && (
          <div className="flex items-center gap-3">
            <div className="size-9 rounded-xl bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center shadow-lg shadow-primary-500/20">
              <Boxes size={22} className="text-white" />
            </div>
            <span className="font-bold text-lg tracking-tight text-white">GesStock</span>
          </div>
        )}
        <button
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        {navItems.reduce<React.ReactElement[]>((acc, { to, icon: Icon, label, adminOnly }) => {
          if (adminOnly && !isAdmin) return acc
          acc.push(
            <NavLink
              key={to}
              to={to}
