...
export const exportInventoryToPDF = (report: InventoryReport) => {
    ...
    doc.text(`Valeur Totale : ${report.summary.totalValue.toLocaleString('fr-FR').replace(/\s/g, ' ')} F CFA`, 14, 46)

    // Alert indicator
    if (report.summary.lowStockProducts > 0 || report.summary.outOfStock > 0) {
        doc.setTextColor(220, 38, 38)
        doc.text(`Alerte : ${report.summary.lowStockProducts} stock(s) faible(s), ${report.summary.outOfStock} rupture(s)`, 14, 52)
        doc.setTextColor(0, 0, 0)
    }

    const tableColumn = ["SKU", "Produit", "Catégorie", "Stock Actuel", "Seuil Min", "Prix U.", "Valeur", "Statut"]
...
export const exportMovementsToPDF = (report: MovementReport, dateRangeStr: string) => {
    ...
    doc.setFontSize(10)
    doc.text(`Généré le : ${date}`, 14, 40)
    doc.text(`Volume Total : ${report.summary.totalMovements} actions (${report.summary.totalIn} entrées / ${report.summary.totalOut} sorties)`, 14, 46)

    const tableColumn = ["Date", "Produit", "SKU", "Type", "Quantité", "Référence", "Note"]
    const tableRows: (string | number)[][] = []

    report.movements.forEach(m => {
        const typeMap: Record<string, string> = { 'IN': '+ Entrée', 'OUT': '- Sortie', 'ADJUSTMENT': 'Ajustement', 'TRANSFER': 'Transfert' }
        
        // Formatage de la taille des données encombrantes pour éviter de surcharger le document
        const shortProductName = m.productName.length > 20 ? `${m.productName.substring(0, 20)}..` : m.productName
        const shortRef = m.reference && m.reference.length > 12 ? `${m.reference.substring(0, 12)}..` : (m.reference || '-')
