...
function PlanCard({
    ...
    name: string
    plan: { name: string; price: number; productLimit: number; userLimit: number; warehouseLimit: number; features: string[] }
    currentPlan: string
    isActive: boolean
    onSelect: () => void
}) {
    ...
    return (
        <div className={`card p-6 relative ${isActive ? 'ring-2 ring-blue-500' : ''} ${isCurrent ? 'border-blue-500 border-2' : ''}`}>
            {isCurrent && (
            ...
            <div className="mb-4">
                ...
                <div className="flex items-baseline gap-1 mt-1">
                    <span className="text-3xl font-bold text-gray-900">{plan.price.toLocaleString('fr-FR')} F CFA</span>
        ...
        <div className="space-y-6">
                ...
                <div className="space-y-6">
                    <div className="card p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Plan actuel</label>
                                        ...
                                        return (
                                            <div>
                                                <div className="flex justify-between items-center mb-1.5">
                                                    ...
                                                    <span className={`text-sm font-bold ${pct >= 90 ? 'text-red-600' : 'text-gray-600'}`}>
                                                ...
                                                {pct >= 90 && !isUnlimited && (
                                                    <p className="text-xs text-red-500 mt-1">⚠️ Quota presque atteint — passez au plan supérieur</p>
                                                )}
                                            </div>
                                        )
                                    })()}
                                    ...
                                    <div>
                                        <div className="flex justify-between items-center mb-1.5">
                                            ...
                                            <span className="text-sm font-bold text-gray-600">
                                                {(plans[billing.plan].warehouseLimit === Infinity || plans[billing.plan].warehouseLimit === null) ? 'Illimité' : `Max ${plans[billing.plan].warehouseLimit}`}
                                            </span>
                                        </div>
                                    </div>
                                </>
            ...
            {activeTab === 'billing' && (
                <div className="space-y-6">
                                ...
                                {!billing.stripeSubscriptionId && (
                                    <div className="p-4 bg-blue-50 rounded-lg">
                                        <p className="text-sm text-blue-800">
                                            Vous êtes actuellement en essai ou sur le plan gratuit.
                                            Sélectionnez un plan ci-dessous pour vous abonner.
                                        </p>
                                    </div>
                                )}
                            </div>
                    ...
                    {isAdmin && (
                        <div className="card p-6">
                            <h3 className="text-lg font-semibold text-gray-900 mb-4">Changer de plan (Paiement Mobile Money)</h3>
                            <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 mb-6">
                                ...
                                <p className="text-sm text-blue-800 mb-4">
                                    Pour activer ou renouveler votre compte Pro / Enterprise, veuillez effectuer un dépôt Mobile Money (Wave, Orange, MTN ou Moov) sur l'un de nos numéros professionnels. L'activation de votre compte se fera manuellement dans les minutes qui suivent.
                                </p>
                                <ul className="list-disc list-inside text-sm text-blue-800 font-medium space-y-1 mb-4">
                                    <li>Wave CI : <strong>+225 00 00 00 00 00</strong></li>
                                    <li>Orange Money : <strong>+225 00 00 00 00 00</strong></li>
                                </ul>
                                <a 
                            ...
                            {plansLoading ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <PlanCard
                                        ...
                                        isActive={false}
                                        onSelect={() => toast.success('Pour rester sur Starter, aucune action requise.')}
                                    />
                                    <PlanCard
                                        name="Pro"
                                        plan={plans.pro}
                                        currentPlan={billing?.plan || 'starter'}
                                        isActive={true}
                                        onSelect={() => toast('Effectuez le dépôt puis contactez-nous sur WhatsApp !', { icon: '📲' })}
                                    />
                                    <PlanCard
                                        name="Enterprise"
                                ...
                                <table className="w-full">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th className="text-left text-xs font-medium text-gray-500 uppercase px-4 py-3">Montant</th>
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <div className="card p-6">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                            <div>
                                ...
                                <p className="text-sm text-gray-500">Utilisez ces clés pour connecter votre site e-commerce ou logiciel comptable.</p>
                            </div>
                            <div className="flex gap-2">
                                ...
                                >
                                    {createApiKey.isPending ? <Loader2 size={16} className="animate-spin" /> : <Plus size={16} />}
                                    Générer
                                </button>
                            </div>
                        </div>
                        ...
                        {generatedKey && (
                            <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl animate-in zoom-in-95 duration-300">
                                ...
                                <p className="text-xs text-amber-700 mb-3">Pour des raisons de sécurité, cette clé ne sera plus jamais affichée.</p>
                                <div className="flex items-center gap-2">
                            ...
                            <table className="w-full">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="text-left text-xs font-medium text-gray-500 uppercase px-4 py-3">Dernière utilisation</th>
                                    ...
                                    {apiKeys?.data?.map((key) => (
                                        <tr key={key.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-right">
                                                <button 
                                                    onClick={() => {
                                                        if (confirm('Révoquer cette clé ? Les services l\'utilisant ne pourront plus se connecter.')) {
                    ...
                    <div className="card p-6 bg-blue-50 border-blue-100">
                        ...
                        <p className="text-xs text-blue-800 leading-relaxed space-y-2">
                            ...
                            <code className="bg-white border border-blue-200 px-1 py-0.5 rounded text-blue-600 font-mono">X-API-Key: VOTRE_CLE_ICI</code>
                        </p>
