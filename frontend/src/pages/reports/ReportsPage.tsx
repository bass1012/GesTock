...
export default function ReportsPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'inventory' | 'movements' | 'bi' | 'forecasts'
  >('overview')
  ...
  const isLoading =
    ...
    isLoadingMovements ||
    isLoadingInventory ||
    (isPro && (isLoadingExpiry || isLoadingSlow || isLoadingForecasts))
  ...
  const processChartData = () => {
    ...
    for (let i = days; i >= 0; i--) {
      ...
      const dayMovements =
        movementsResponse?.movements?.filter((m) => isSameDay(new Date(m.createdAt), date)) || []

      chartData.push({
        date: format(date, 'd MMM', { locale: fr }),
        Sorties: dayMovements
          .filter((m) => m.type === 'OUT')
          .reduce((sum, m) => sum + m.quantity, 0),
        Entrées: dayMovements
          .filter((m) => m.type === 'IN')
          .reduce((sum, m) => sum + m.quantity, 0),
      })
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handlePDFInventory}
            className="btn bg-rose-50 text-rose-600 hover:bg-rose-100 flex items-center gap-2 border border-rose-200"
          >
            <FileText size={18} />
      ...
      {activeTab === 'overview' && stats && (
        <div className="space-y-6">
              ...
              <div className="space-y-3">
                {stats.topProducts.map((product, i) => (
                  <div key={product.id} className="flex items-center gap-3">
                    <span className="size-6 rounded-full bg-blue-100 text-blue-600 text-xs font-bold flex items-center justify-center">
                      {i + 1}
                    </span>
          ...
          {stats.lowStockCount > 0 && (
            <div className="card p-6 shadow-[inset_3px_0_0_0_theme(colors.red.500)] bg-red-50">
              <div className="flex items-start gap-3">
                <AlertTriangle className="text-red-500 mt-0.5" size={20} />
                <div>
                  <h3 className="font-semibold text-red-900">Alertes de stock</h3>
                  <p className="text-sm text-red-700 mt-1">
                    {stats.lowStockCount} produit{stats.lowStockCount > 1 ? 's sont' : ' est'} en
                    dessous du seuil minimum. Consultez la page Inventaire pour plus de détails.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                {expiryResponse?.map((item) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p
                        className={`text-sm font-bold ${item.daysRemaining <= 7 ? 'text-red-500' : 'text-amber-600'}`}
                      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="card p-4 bg-gradient-to-br from-emerald-50 to-white border-emerald-100">
              ...
              <p className="text-xs text-emerald-700 mt-1">
                Stock idéal. Concentrez vos achats sur ces catégories à forte rotation.
              </p>
            </div>
          </div>
        </div>
                    ...
                    {forecastsResponse?.map((item) => {
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-zinc-50/50 transition-colors">
                          ...
                          <td className="px-5 py-3">
                            <span
                              className={`text-xs font-semibold px-2 py-1 rounded-full ${urgencyStyle}`}
                            >
                              {urgencyLabel}
                            </span>
                          </td>
