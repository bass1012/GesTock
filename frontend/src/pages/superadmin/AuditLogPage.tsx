...
const formatAuditDate = (d: string) =>
  format(new Date(d), "dd MMM yyyy '\u00e0' HH:mm:ss", { locale: fr })
...
const ACTION_META: Record<string, { label: string; color: string; bg: string; icon: any }> = {
  ...
  API_KEY_GENERATED: {
    label: 'Clé API générée',
    color: 'text-teal-400',
    bg: 'bg-teal-500/10 border-teal-500/20',
    icon: Key,
  },
  API_KEY_REVOKED: {
    label: 'Clé API révoquée',
    color: 'text-red-400',
    bg: 'bg-red-500/10 border-red-500/20',
    icon: Key,
  },
  ...
  SUPPLIER_CREATED: {
    label: 'Fournisseur créé',
    color: 'text-green-400',
    bg: 'bg-green-500/10 border-green-500/20',
    icon: Briefcase,
  },
  SUPPLIER_UPDATED: {
    label: 'Fournisseur modifié',
    color: 'text-blue-400',
    bg: 'bg-blue-500/10 border-blue-500/20',
    icon: Briefcase,
  },
  SUPPLIER_DELETED: {
    label: 'Fournisseur supprimé',
    color: 'text-red-400',
    bg: 'bg-red-500/10 border-red-500/20',
    icon: Briefcase,
  },
  ...
  WAREHOUSE_UPDATED: {
    label: 'Entrepôt modifié',
    color: 'text-blue-400',
    bg: 'bg-blue-500/10 border-blue-500/20',
    icon: Package,
  },
  WAREHOUSE_DELETED: {
    label: 'Entrepôt supprimé',
    color: 'text-red-400',
    bg: 'bg-red-500/10 border-red-500/20',
    icon: Package,
  },
...
export default function AuditLogPage({ secret }: { secret: string | null }) {
  ...
  useEffect(() => {
    fetchLogs(1)
  }, [filterTenant, filterAction, filterPeriod])
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <KpiCard
            ...
            value={stats.summary.logins24h}
            icon={<LogIn className="text-blue-500" />}
            color="blue"
          />
          <KpiCard
            label="Suspensions (7j)"
            value={stats.summary.suspensions7d}
            icon={<Ban className="text-red-500" />}
            color="red"
          />
          <KpiCard
            label="Abonnements (7j)"
            value={stats.summary.subscriptions7d}
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {(filterTenant || filterAction || filterPeriod !== '7d') && (
                <span className="ml-1 size-2 bg-amber-400 rounded-full" />
              )}
            </button>
            <button
              onClick={() => fetchLogs(1)}
              ...
              {displayedLogs.map((log) => {
                ...
                return (
                  <div key={log.id} className="flex gap-4 group">
                    {/* Icône timeline */}
                    <div
                      className={`relative z-10 flex-shrink-0 size-12 rounded-xl border flex items-center justify-center ${meta.bg}`}
                    >
                    ...
                    >
                      <div className="flex flex-wrap items-center gap-3 mb-1">
                        ...
                        {!log.user && log.userId === 'superadmin' && (
                          <span className="text-[10px] px-1.5 py-0.5 bg-primary-500/10 text-primary-400 border border-primary-500/20 rounded font-black">
                            QG
                          </span>
                        )}
                      </div>
