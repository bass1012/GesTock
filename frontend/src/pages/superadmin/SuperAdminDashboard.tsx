...
const formatPeriodDate = (d: string) => format(new Date(d), 'dd MMM yyyy', { locale: fr })

export default function SuperAdminDashboard() {
  const [tenants, setTenants] = useState<any[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [searchTerm, setSearchTerm] = useState('')
  const [activeFilter, setActiveFilter] = useState<'all' | 'recent' | 'expiring' | 'suspended'>(
    'all',
  )
  ...
  const stats = useMemo(
    () => ({
      ...
      expiringSoon: tenants.filter((t) => {
        ...
        const diff = Date.parse(t.currentPeriodEnd) - Date.now()
    ...
    switch (activeFilter) {
      ...
      case 'expiring':
        ...
        result = result
          ...
          .sort((a, b) => Date.parse(a.currentPeriodEnd) - Date.parse(b.currentPeriodEnd))
        break
      case 'suspended':
        // Comptes suspendus
  ...
  const handleUpdatePlan = async (id: string, plan: string, durationMonths: number) => {
    setConfirmConfig({
      ...
      message: `Activer le plan ${plan.toUpperCase()} (${durationMonths} mois) ?`,
      onConfirm: async () => {
        try {
  ...
  const handleToggleStatus = async (id: string, isSuspended: boolean) => {
    ...
    setConfirmConfig({
      ...
      message: `Voulez-vous vraiment ${action} ce compte ?`,
      onConfirm: async () => {
        try {
  ...
  const handleToggleApi = async (id: string, apiEnabled: boolean) => {
    try {
      await axios.put(
        `/api/v1/superadmin/tenants/${id}/api`,
        {},
        {
          headers: { Authorization: `Bearer ${secret}` },
        },
  ...
  const recentCutoff = nowMs - sevenDaysMs
  const recentCount = tenants.filter((t) => Date.parse(t.createdAt) >= recentCutoff).length
  const expiringCount = tenants.filter((t) => {
    if (!t.currentPeriodEnd) return false
    const diff = Date.parse(t.currentPeriodEnd) - nowMs
  ...
  return (
    <div className="min-h-screen bg-[#0B0F1A] text-zinc-300 font-sans pb-20">
      ...
      <main className="p-8 max-w-7xl mx-auto space-y-10">
        {/* ── Onglet Audit ── */}
        {activeTab === 'audit' && <AuditLogPage secret={secret} />}
              ...
              <div className="flex items-center gap-4 text-xs font-mono text-zinc-500 uppercase">
                ...
                {activeFilter !== 'all' ? (
                  <span className="text-primary-400">
                    Filtre actif: {filteredTenants.length} résultat(s)
                  </span>
                ) : (
                  <span>
                    Affichage de {filteredTenants.length} boutique(s) sur {tenants.length}
                  </span>
                )}
              </div>
            </div>
                ...
                >
                  ...
                  <div className="p-8 flex flex-col xl:flex-row gap-8 items-start xl:items-center">
                    ...
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="flex flex-wrap gap-3 pt-4">
                        <InfoBadge
                          icon={<Users size={12} />}
                          label="Usage"
                          value={`${t.usersCount} utilisateurs`}
                        />
                        <InfoBadge
                          icon={<Clock size={12} />}
                          label="Échéance"
                          value={
                            t.currentPeriodEnd ? formatPeriodDate(t.currentPeriodEnd) : 'Indéfini'
                          }
                          highlight={Date.parse(t.currentPeriodEnd) < nowMs}
                        />
                        <InfoBadge
                          icon={<Key size={12} />}
                          label="API"
                    ...
                    <div className="w-full xl:w-auto grid grid-cols-2 gap-2">
                      ...
                      <PlanButton
                        onClick={() => handleUpdatePlan(t.id, 'starter', 1)}
                        label="Starter +1 M"
                      />
                      <PlanButton
                        onClick={() => handleUpdatePlan(t.id, 'starter', 12)}
                        label="Starter +12 M"
                      />
                      <PlanButton
                        onClick={() => handleUpdatePlan(t.id, 'pro', 1)}
                        label="Pro +1 M"
                        color="primary"
                        highlight
                      />
                      <PlanButton
                        onClick={() => handleUpdatePlan(t.id, 'pro', 12)}
                        label="Pro +12 M"
                        color="primary"
                      />
                      <PlanButton
                        onClick={() => handleUpdatePlan(t.id, 'enterprise', 1)}
                        label="Ent +1 M"
                        color="amber"
                      />
                      <PlanButton
                        onClick={() => handleUpdatePlan(t.id, 'enterprise', 12)}
                        label="Ent +12 M"
                        color="amber"
                      />
                    </div>
...
function InfoBadge({
  ...
  return (
    <div
      className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-[11px] font-medium ${
        highlight
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start">
        ...
        <span className={`text-[10px] font-mono ${active ? 'text-white/80' : 'opacity-60'}`}>
          {count} élément(s)
        </span>
      </div>
      {active && <div className="ml-2 size-2 bg-white rounded-full animate-pulse" />}
    </button>
  )
}
...
function TenantUsers({ tenantId, secret }: { tenantId: string; secret: string | null }) {
  ...
  const handleReset = async (userId: string, userName: string) => {
    setConfirmConfig({
      ...
      message: `Réinitialiser le mot de passe de ${userName} ?`,
      onConfirm: async () => {
        try {
          const { data } = await axios.post(
            `/api/v1/superadmin/users-by-id/${userId}/reset-password`,
            {},
            {
              headers: { Authorization: `Bearer ${secret}` },
            },
  ...
  const handleUpdateRole = async (userId: string, userName: string, newRole: string) => {
    try {
      ...
      await axios.put(
        `/api/v1/superadmin/users-by-id/${userId}/role`,
        { role: newRole },
        {
          headers: { Authorization: `Bearer ${secret}` },
        },
    ...
    <div className="border-t border-zinc-800">
      ...
      {isOpen && (
        <div className="px-8 pb-8 animate-in duration-300">
          {isLoading ? (
            <div className="py-4 text-center text-xs font-mono animate-pulse">
              Scan des profils…
            </div>
          ) : (
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex flex-wrap gap-2">
                        ...
                        {user.mustChangePassword && (
                          <span className="text-[9px] px-1.5 py-0.5 bg-amber-500/10 text-amber-500 rounded border border-amber-500/20 font-black italic">
                            CHANGEMENT REQUIS
                          </span>
                        )}
                      </div>
