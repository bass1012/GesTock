  ...
  for (const tenant of tenants) {
    ...
    try {
      ...
      const warehouses = (await prisma.$queryRawUnsafe(
        `SELECT id FROM "${schemaName}".warehouses WHERE name = 'Dépôt Principal'`,
      )) as any[]
      ...
      const products = (await prisma.$queryRawUnsafe(
        `SELECT id, current_stock FROM "${schemaName}".products`,
      )) as any[]
      ...
      await prisma.$executeRawUnsafe(
        `UPDATE "${schemaName}".stock_movements SET warehouse_id = $1::uuid WHERE warehouse_id IS NULL`,
        warehouseId,
      )
      await prisma.$executeRawUnsafe(
        `UPDATE "${schemaName}".sales SET warehouse_id = $1::uuid WHERE warehouse_id IS NULL`,
        warehouseId,
      )
      await prisma.$executeRawUnsafe(
        `UPDATE "${schemaName}".purchase_orders SET warehouse_id = $1::uuid WHERE warehouse_id IS NULL`,
        warehouseId,
      )
