...
export const authService = {
  async register(data: {
    ...
    return {
      ...
      refreshToken,
      message:
        'Inscription réussie. Un email de vérification a été envoyé. Veuillez vérifier votre email.',
    }
  },
  ...
  async login(email: string, password: string, tenantSlug?: string) {
    ...
    if (lockoutUntil && lockoutUntil > new Date()) {
      throw new UnauthorizedError(
        `Compte verrouillé. Réessayez après ${lockoutUntil.toLocaleString()}`,
      )
    }
  ...
  async resendVerificationEmail(email: string) {
    // Find user by email (find first since multiple tenants could have same email)
    const user = await prisma.user.findFirst({
      where: { email },
    })
    ...
    if (!user) {
      ...
      return { success: true, message: 'Si cet email existe, un lien sera envoyé.' }
    }

    if (user.emailVerified) {
      return { success: true, message: 'Cet email est déjà vérifié.' }
    }
