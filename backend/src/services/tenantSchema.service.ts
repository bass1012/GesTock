export function getTenantSchemaQueries(schemaName: string) {
  return [
    ...
    `CREATE TABLE IF NOT EXISTS "${schemaName}".supplier_return_items (
                id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                return_id UUID NOT NULL REFERENCES "${schemaName}".supplier_returns(id) ON DELETE CASCADE,
                product_id UUID NOT NULL REFERENCES "${schemaName}".products(id),
                quantity INT NOT NULL,
