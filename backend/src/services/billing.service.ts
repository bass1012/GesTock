...
export const PLANS = {
  starter: {
    ...
    userLimit: 2,
    warehouseLimit: 1,
    features: ['Produits limités à 200', '2 utilisateurs', '1 entrepôt', 'Support email'],
  },
  pro: {
    name: 'Pro',
    price: 60000,
...
export const billingService = {
  ...
  async getBillingInfo(tenantId: string): Promise<BillingInfo | null> {
    ...
    const endDate = config.subscriptionEndDate
      ? new Date(config.subscriptionEndDate)
      : new Date(tenant.createdAt.getTime() + 14 * 24 * 60 * 60 * 1000) // 14 days trial Default
