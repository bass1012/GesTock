...
export const reportController = {
  ...
  async exportInventoryPDF(req: Request, res: Response, next: NextFunction) {
    try {
      const tenantSlug = req.tenantSlug as string
      ...
      const report = await reportService.getInventoryReport(tenantSlug, {
        ...
        status: status as string | undefined,
        page: 1,
        limit: 1000, // Export standard size
      })

      res.setHeader('Content-Type', 'application/pdf')
      res.setHeader('Content-Disposition', 'attachment; filename="inventaire.pdf"')

      await pdfService.generateInventoryReportPDF(report, res)
    } catch (error) {
      next(error)
    }
  },

  async exportMovementsPDF(req: Request, res: Response, next: NextFunction) {
    try {
      const tenantSlug = req.tenantSlug as string
      ...
      await pdfService.generateMovementReportPDF(report, res)
    } catch (error) {
      next(error)
    }
  },
